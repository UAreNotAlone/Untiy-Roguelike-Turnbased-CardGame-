...
public class Whiskey: CardItem
{
    ...
    public override void OnEndDrag(PointerEventData eventData)
    {
        if (TryUseCard_Mana() == true)
        {
            ...
            Debug.Log("whiskey is used");
            //获取所有头部debufff并随机删除一个
            
            List<Buff> debuffs = new List<Buff>();
            foreach (Transform buff in buffIconManager.buffs)
            {
                if (buff.GetComponent<Buff>().isDebuff == true && buff.GetComponent<Buff>().buffType == Buff.BuffType.headBuff)
                {
                    debuffs.Add(buff.GetComponent<Buff>());
                }
            }
            if (debuffs.Count != 0)//当有头部debuff可以删除时进行这样一个删除的工作
            {
