...
public class BeerBuff : CardItem
{
    ...
    public override void OnEndDrag(PointerEventData eventData)
    {
        if (TryUseCard_Mana() == true)
        {
            ...
            Debug.Log("Beer is used");
            
            //获取所有腿部debufff并随机删除一个
            List<Buff> debuffs = new List<Buff>();
            foreach(Transform buff in buffIconManager.buffs)
            {
                if(buff.GetComponent<Buff>().isDebuff == true && buff.GetComponent<Buff>().buffType == Buff.BuffType.legsBuff)
                {
                    debuffs.Add(buff.GetComponent<Buff>());
                }
            }
            if (debuffs.Count != 0)//当有腿部debuff可以删除时进行这样一个删除的工作
            {
